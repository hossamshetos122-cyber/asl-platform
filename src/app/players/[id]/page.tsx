    ...
    <>
            ...
            <div className="flex-1 text-center sm:text-right">
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-1.5 mb-2">
                {player.jerseyNumber != null && (
                  <span className="inline-flex h-7 items-center rounded bg-gold px-2 font-num text-sm font-black text-bg">{player.jerseyNumber}</span>
                )}
                <span className="rounded bg-gold/8 border border-gold/15 px-2 py-0.5 font-body text-[11px] font-bold text-gold">
                  {POSITION_LABELS[player.position] ?? player.position}
                </span>
              </div>
              <h1 className="font-display text-2xl sm:text-3xl font-black text-text">{player.name}</h1>
              {player.team && (
                <Link href={`/teams/${player.team.id}`} className="mt-2 inline-flex items-center gap-2 rounded-lg border border-line bg-surface-elevated px-3 py-1.5 font-body text-[12px] font-bold text-text-dim transition-colors hover:border-line-gold hover:text-gold">
      ...
      <main className="page-container page-padding">
        ...
        <div className="mb-5 grid grid-cols-3 gap-2">
          ...
          <div className="rounded-xl border border-line bg-surface p-4 text-center">
            <div className="font-num text-sm font-bold text-gold leading-tight">{POSITION_LABELS[player.position] ?? player.position}</div>
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            {player.team && (
              <div>
                ...
                <dd className="mt-1 font-body text-[13px]"><Link href={`/teams/${player.team.id}`} className="text-gold hover:text-gold-bright transition-colors font-bold">{player.team.name}</Link></dd>
              </div>
            )}
          </dl>
        </div>
